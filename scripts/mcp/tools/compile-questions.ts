...
function build(questions: ParsedQuestion[], answers: Map<number, ParsedAnswer>) {
	...
	for (const n of answers.keys()) {
		if (!questionNums.has(n))
			errors.push(`answers.md has Q${n} with no matching question`);
	}
		...
		"compile_questions",
		{
			...
			inputSchema: {
				questionsPath: z
					.string()
					.describe(
						"Absolute path to questions.md (stems + lettered options, no answer key)."
					),
				answersPath: z
					.string()
					.describe(
						"Absolute path to answers.md (correct letters/text + optional explanations)."
					),
				slug: z
					.string()
					.describe(
						"2-3 word kebab summary for the output filename, e.g. 'tcp-handshake'."
					),
				outDir: z
					.string()
					.optional()
					.describe(
						"Output directory, relative to repo root. Default: pending-questions."
					),
			},
		},
