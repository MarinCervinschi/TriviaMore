...
export function FlashcardManager() {
  ...
  const availableClasses = useMemo(() => {
    if (!selectedCourseId) {
      return availableCourses.flatMap((course) => course.classes)
    }
    return availableCourses.find((course) => course.id === selectedCourseId)?.classes || []
  }, [availableCourses, selectedCourseId])

  const availableSections = useMemo(() => {
    if (!selectedClassId) {
      return availableClasses.flatMap((cls) => cls.sections)
    }
    return availableClasses.find((cls) => cls.id === selectedClassId)?.sections || []
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-bold">Flashcard Management</h2>
      ...
      {showForm && (
        <Card>
          <CardHeader>
            <CardTitle>{editingQuestion ? "Edit Flashcard" : "Add New Flashcard"}</CardTitle>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <Label htmlFor="content">Flashcard Question</Label>
                <Textarea
                  id="content"
      ...
      {filteredQuestions.length === 0 && (
        <div className="text-center py-8 text-gray-500">No flashcards found matching your criteria.</div>
      )}
    </div>
  )
}
