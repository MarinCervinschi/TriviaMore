import { PrismaClient, Role, QuestionType, Difficulty, CourseType, QuizMode } from '@prisma/client'
...
async function main() {
    ...
    const standardMode = await prisma.evaluationMode.create({
        data: {
            name: 'Standard',
            description: 'Modalità di valutazione standard: 1 punto per risposta corretta, 0 per sbagliata',
    ...
    const penaltyMode = await prisma.evaluationMode.create({
        data: {
            name: 'Con Penalità',
            description: 'Modalità con penalità: 1 punto per risposta corretta, -0.25 per sbagliata',
            correctAnswerPoints: 1.0,
    ...
    await prisma.question.createMany({
        data: [
            {
                content: 'Quale tipo di dato viene utilizzato per memorizzare numeri interi in C?',
                questionType: QuestionType.MULTIPLE_CHOICE,
                options: ['float', 'int', 'char', 'double'],
                correctAnswer: ['int'],
                explanation: 'Il tipo di dato "int" è utilizzato per memorizzare numeri interi in C.',
            ...
            },
            {
                ...
                correctAnswer: ['false'],
                explanation: 'Il C è considerato un linguaggio di medio livello, non ad alto livello.',
            ...
            },
            {
                ...
                explanation: 'La funzione printf() è utilizzata per stampare output formattato in C.',
                difficulty: Difficulty.EASY,
                sectionId: programmingBasicsSection.id,
            },
        ],
        ...
        data: [
            {
                ...
                correctAnswer: ['Stack'],
                explanation: 'Lo Stack segue il principio LIFO: l\'ultimo elemento inserito è il primo ad essere rimosso.',
                difficulty: Difficulty.MEDIUM,
                sectionId: algorithmsSection.id,
            },
            {
                content: 'L\'algoritmo di ordinamento Bubble Sort ha complessità O(n²) nel caso peggiore.',
                questionType: QuestionType.TRUE_FALSE,
                correctAnswer: ['true'],
    ...
    await prisma.question.createMany({
        data: [
            {
                content: 'Quale diagramma UML è utilizzato per modellare il comportamento dinamico di un sistema?',
                questionType: QuestionType.MULTIPLE_CHOICE,
                options: ['Diagramma delle classi', 'Diagramma di sequenza', 'Diagramma dei casi d\'uso', 'Diagramma dei componenti'],
                correctAnswer: ['Diagramma di sequenza'],
                explanation: 'I diagrammi di sequenza mostrano l\'interazione tra oggetti nel tempo, modellando il comportamento dinamico.',
                difficulty: Difficulty.MEDIUM,
            ...
            },
            {
                ...
                questionType: QuestionType.TRUE_FALSE,
                correctAnswer: ['true'],
                explanation: 'La composizione rappresenta una relazione "parte-di" forte dove le parti non possono esistere senza il tutto.',
                difficulty: Difficulty.EASY,
                sectionId: umlSection.id,
            },
        ],
        ...
        data: [
            {
                ...
                explanation: 'Il pattern Singleton garantisce che una classe abbia una sola istanza e fornisce un punto di accesso globale.',
                difficulty: Difficulty.EASY,
                sectionId: designPatternsSection.id,
            },
            {
                content: 'Il pattern Observer implementa una relazione uno-a-molti tra oggetti.',
                questionType: QuestionType.TRUE_FALSE,
                correctAnswer: ['true'],
                explanation: 'L\'Observer pattern definisce una relazione uno-a-molti: quando un oggetto cambia stato, tutti i suoi dipendenti vengono notificati.',
                difficulty: Difficulty.MEDIUM,
                sectionId: designPatternsSection.id,
            },
        ],
    ...
    await prisma.question.createMany({
        data: [
            {
                content: 'Quale comando SQL viene utilizzato per recuperare dati da una tabella?',
                questionType: QuestionType.MULTIPLE_CHOICE,
                options: ['INSERT', 'UPDATE', 'SELECT', 'DELETE'],
                correctAnswer: ['SELECT'],
                explanation: 'Il comando SELECT è utilizzato per interrogare e recuperare dati da una o più tabelle.',
                difficulty: Difficulty.EASY,
            ...
            },
            {
                ...
                questionType: QuestionType.TRUE_FALSE,
                correctAnswer: ['true'],
                explanation: 'La clausola WHERE specifica le condizioni che i record devono soddisfare per essere inclusi nel risultato.',
                difficulty: Difficulty.EASY,
                sectionId: sqlSection.id,
            },
            {
                ...
                questionType: QuestionType.SHORT_ANSWER,
                correctAnswer: ['SELECT colonne FROM tabella', 'SELECT * FROM tabella', 'SELECT campo FROM tabella'],
                explanation: 'La sintassi base è: SELECT colonne FROM nome_tabella [WHERE condizioni]',
                difficulty: Difficulty.MEDIUM,
                sectionId: sqlSection.id,
            },
        ],
        ...
        data: [
            {
                ...
                correctAnswer: ['2NF'],
                explanation: 'La seconda forma normale (2NF) elimina le dipendenze parziali dalla chiave primaria.',
                difficulty: Difficulty.MEDIUM,
            ...
            },
            {
                ...
                questionType: QuestionType.TRUE_FALSE,
                correctAnswer: ['true'],
                explanation: 'Le forme normali sono progressive: per essere in 3NF, una tabella deve essere anche in 2NF e 1NF.',
                difficulty: Difficulty.MEDIUM,
                sectionId: normalizationSection.id,
            },
        ],
    ...
    await prisma.question.createMany({
        data: [
            {
                content: 'Qual è il risultato del prodotto scalare di due vettori perpendicolari?',
                questionType: QuestionType.MULTIPLE_CHOICE,
            ...
            },
            {
                ...
                questionType: QuestionType.TRUE_FALSE,
                correctAnswer: ['true'],
                explanation: 'Un vettore può essere rappresentato come una matrice colonna con n righe e 1 colonna.',
                difficulty: Difficulty.EASY,
                sectionId: vectorsSection.id,
            },
        ],
        ...
        data: [
            {
                ...
                explanation: 'ROI sta per Return on Investment, ovvero il ritorno sull\'investimento.',
                difficulty: Difficulty.EASY,
            ...
            },
            {
                ...
                questionType: QuestionType.TRUE_FALSE,
                correctAnswer: ['true'],
                explanation: 'Il bilancio deve sempre rispettare l\'equazione fondamentale: Attivo = Passivo + Patrimonio Netto.',
                difficulty: Difficulty.MEDIUM,
                sectionId: businessFundamentalsSection.id,
            },
        ],
        ...
        data: [
            {
                ...
                explanation: 'Le 4P del marketing mix sono: Product, Price, Place, Promotion. People non è tra queste.',
                difficulty: Difficulty.MEDIUM,
    ...
    const programmingQuestions = allQuestions.filter(q => q.sectionId === programmingBasicsSection.id)
    ...
    const algebraQuestionsList = allQuestions.filter(q => q.sectionId === vectorsSection.id)
    const businessQuestionsList = allQuestions.filter(q => q.sectionId === businessFundamentalsSection.id)
