...
function BrowsePage() {
	...
	const availableAreas = useMemo(
		() => [...new Set(departments.flatMap(d => (d.area ? [d.area] : [])))].sort(),
		[departments]
	);
			...
			>
				...
				>
					...
					<p className="text-muted-foreground mt-3 max-w-2xl text-base sm:text-lg">
						Naviga la struttura accademica completa: dipartimenti, corsi di laurea,
						insegnamenti e sezioni di esercizio. Tutto in un unico posto.
					</p>
				</motion.div>
				...
				<div className="container">
					...
					>
						...
						<p className="text-muted-foreground text-sm leading-relaxed sm:text-base">
							Ogni contenuto segue la struttura accademica UNIMORE. Naviga la gerarchia
							per trovare in pochi click sezioni e quiz su cui esercitarti.
						</p>
					</motion.div>
			...
			<section ref={dataRef} className="container pt-4 pb-16">
				...
				>
					...
					<div className="flex flex-wrap items-end justify-between gap-3">
						...
						<p className="text-muted-foreground flex flex-wrap items-baseline gap-x-3 gap-y-1 text-sm">
...
function DotSep() {
	return (
		<span
			aria-hidden
			className="bg-muted-foreground/30 hidden h-1 w-1 rounded-full sm:block"
		/>
	);
}
...
function AreaPill({
	...
	return (
		...
		>
			{accent && <span className={cn("h-2 w-2 rounded-full", accent)} aria-hidden />}
			{label}
		</button>
	);
}
