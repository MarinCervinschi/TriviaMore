...
const REPORT_REASON_OPTIONS = [
	{
		id: "errata",
		label: "Errata",
		description: "La domanda o la risposta contiene un errore",
	},
	{
		id: "imprecisa",
		label: "Imprecisa",
		description: "La formulazione è ambigua o poco chiara",
	},
	{
		id: "fuori_contesto",
		label: "Fuori contesto",
		description: "La domanda non appartiene a questa sezione",
	},
			...
			<div className="space-y-2">
				...
				<div className="bg-muted/30 flex items-center gap-3 rounded-2xl border p-4">
					...
					<div>
						...
						<p className="text-muted-foreground text-xs">
							{formatFileSize(submitted.file_size)}
						</p>
					</div>
				</div>
				{submitted.comment && (
					<p className="text-muted-foreground text-sm">{submitted.comment}</p>
				)}
			</div>
		);
	}
	...
	return (
		<div className="space-y-4 rounded-2xl border border-orange-500/20 bg-orange-500/5 p-5">
			...
			{content.type === "section" ? (
				<SectionEditor content={content} onChange={updated => setContent(updated)} />
			) : content.type === "questions" ? (
				<QuestionsEditor content={content} onChange={updated => setContent(updated)} />
			) : null}
		...
		<div className="border-primary/20 bg-primary/5 space-y-4 rounded-2xl border p-5">
			...
			<div className="space-y-1.5">
				...
				<Textarea
					...
					className={cn(
						"rounded-xl",
						hasAltro && !comment.trim() && "border-red-500/50"
					)}
				/>
			</div>
	...
	return (
		<div className="space-y-2">
			...
			<div className="bg-muted/30 space-y-3 rounded-2xl border p-4">
				...
				{report.comment && (
					<p className="text-foreground/90 text-sm">{report.comment}</p>
				)}
...
function QuestionPreview({
	...
	return (
		<div className="bg-muted/30 space-y-2 rounded-2xl border p-3">
			<div className="flex items-center justify-between">
