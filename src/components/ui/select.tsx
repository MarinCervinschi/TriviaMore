...
>(({ className, children, ...props }, ref) => (
	<SelectPrimitive.Trigger
		ref={ref}
		className={cn(
			"border-input ring-offset-background placeholder:text-muted-foreground focus:ring-ring flex h-10 w-full items-center justify-between rounded-xl border bg-transparent px-3 py-2 text-sm whitespace-nowrap shadow-sm focus:ring-2 focus:ring-offset-2 focus:outline-none disabled:cursor-not-allowed disabled:opacity-50 [&>span]:line-clamp-1",
			className
		)}
		{...props}
	>
		...
		<SelectPrimitive.Icon asChild>
			<AltArrowDownIcon className="h-4 w-4 opacity-50" />
...
>(({ className, ...props }, ref) => (
	<SelectPrimitive.ScrollUpButton
		...
		{...props}
	>
		<AltArrowUpIcon className="h-4 w-4" />
	</SelectPrimitive.ScrollUpButton>
));
...
>(({ className, ...props }, ref) => (
	<SelectPrimitive.ScrollDownButton
		...
		{...props}
	>
		<AltArrowDownIcon className="h-4 w-4" />
	</SelectPrimitive.ScrollDownButton>
));
