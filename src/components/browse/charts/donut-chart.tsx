...
export type DonutDatum = { type: string; label: string; count: number };

// Shared donut: a center total, a slice per datum, and a legend list. The two
// browse donuts differed only by title, unit noun, and the type→color map.
export function DonutChart({
	...
	const config: ChartConfig = data.reduce<ChartConfig>((acc, entry) => {
		acc[entry.type] = {
			label: entry.label,
			color: colors[entry.type] ?? "var(--color-chart-5)",
		};
		return acc;
	}, {});
