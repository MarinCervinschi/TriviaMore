...
export function CampusBarChart({ data }: { data: CampusChartData[] }) {
	...
	const config: ChartConfig = sorted.reduce<ChartConfig>((acc, entry, idx) => {
		acc[entry.campus] = {
			label: entry.label,
	...
	return (
		<ChartCard title="Corsi per campus">
			<ChartContainer
				config={config}
			...
			>
				...
				>
					<PolarGrid
						...
						strokeOpacity={0.5}
					/>
					<PolarAngleAxis type="number" domain={[0, Math.max(max, 1)]} tick={false} />
					<RadialBar
						dataKey="count"
			...
			<ul className="grid grid-cols-2 gap-x-3 gap-y-1.5 text-xs">
				{chartData.map(entry => (
					<li key={entry.campus} className="flex items-center justify-between gap-2">
