...
export function SkeletonAvatar({ className, size = 40 }: Common & { size?: number }) {
	return (
		<Skeleton
...
export function SkeletonChart({
	...
	return (
		<Skeleton className={cn("w-full rounded-2xl", className)} style={{ height }} />
	);
}
				...
				{Array.from({ length: rows }).map((_, r) => (
					...
					>
						{Array.from({ length: columns }).map((__, c) => (
							<Skeleton key={c} className={cn("h-4", c === 0 ? "w-5/6" : "w-1/2")} />
						))}
					</div>
				))}
			</div>
...
export function SkeletonListRow({
	...
	return (
		<div
			className={cn(
				"bg-card flex items-center justify-between gap-3 rounded-2xl border p-4",
				className
			)}
		>
