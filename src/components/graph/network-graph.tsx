...
function NodeCard({
	...
	return (
		<div className="bg-card/95 absolute right-4 bottom-4 z-10 w-[min(20rem,calc(100vw-2rem))] rounded-2xl border p-4 shadow-lg backdrop-blur-sm sm:right-6 sm:bottom-6">
			<div className="flex items-center justify-between gap-2">
				...
				{isPinned && (
					<button
						type="button"
						onClick={onClose}
						className="text-muted-foreground hover:text-foreground -mt-1 -mr-1 cursor-pointer rounded-full p-1 text-xs leading-none"
						aria-label="Chiudi"
					>
						✕
					</button>
			...
			<div className="mt-1 text-sm leading-snug font-semibold">{meta.name}</div>
			{meta.kind === "course" && (
			...
			{isPinned ? (
				<Button asChild size="sm" className="mt-3 w-full">
					{meta.kind === "root" ? (
						...
						<Link to="/browse/$department" params={{ department: meta.code }}>
							Apri dipartimento
						</Link>
					) : (
						<Link
