...
export function SectionsComparison({
	...
	const sectionsWithIssues = studyProgress.filter(s => (s.averageScore ?? 0) < 18);
	const excellentSections = studyProgress.filter(s => (s.averageScore ?? 0) >= 27);
						...
						{sectionsWithIssues.map(item => (
							...
							>
								<div className="min-w-0 flex-1">
									<p className="truncate font-medium">{item.sectionName}</p>
									<p className="text-muted-foreground text-xs">
										{item.className} &bull; {item.quizzesTaken} quiz
									</p>
								</div>
				...
				<div className="bg-card overflow-hidden rounded-2xl border">
					<div className="flex items-center gap-3 border-b p-5">
						...
						<div>
							...
							<p className="text-muted-foreground text-xs">Ottimi risultati (27+)</p>
						</div>
					</div>
					<div className="divide-y">
