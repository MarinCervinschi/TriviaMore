...
export const PUT = auth(async function PUT(
	...
	try {
		...
		const { name, code, description, classYear, position } = body;

		const classData = await AdminService.updateClass(userId, id, {
			name,
			code,
			description,
			classYear: classYear ? Number.parseInt(classYear) : undefined,
			position,
		});
