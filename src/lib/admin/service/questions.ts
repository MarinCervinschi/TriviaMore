...
export async function getAdminQuestionDetail(id: string): Promise<AdminQuestionDetail> {
...
export async function createQuestion(input: QuestionInput) {
	...
	const [question] = await getDb().insert(questions).values(toRow(input)).returning();
	return question;
}
...
export async function updateQuestion(id: string, updates: UpdateQuestionInput) {
	...
	const [question] = await getDb()
		...
		.set({
			...
			explanation:
				updates.explanation === undefined ? undefined : updates.explanation || null,
