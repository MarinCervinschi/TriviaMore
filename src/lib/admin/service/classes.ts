...
export async function getAdminClassDetail(
  ...
  const [sentinel] = await db
    ...
    .from(sections)
    .where(
      and(eq(sections.classId, id), eq(sections.name, EXAM_SIMULATION_SECTION)),
    )
    .limit(1)
...
export async function deleteClass(id: string) {
  ...
  const [{ value: sectionCount }] = await db
    ...
    .from(sections)
    .where(
      and(eq(sections.classId, id), ne(sections.name, EXAM_SIMULATION_SECTION)),
    )
...
export async function updateCourseClass(
  ...
  const [link] = await getDb()
    ...
    .set({
      ...
      catalogueUrl:
        updates.catalogue_url === undefined
          ? undefined
          : updates.catalogue_url || null,
      curriculum:
        updates.curriculum === undefined
          ? undefined
          : updates.curriculum || null,
      position: updates.position,
    })
    .where(
      and(
