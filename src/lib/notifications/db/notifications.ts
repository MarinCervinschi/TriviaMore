...
export async function findNotifications(db: DbOrTx, userId: string, limit: number) {
	return db
		.select()
		.from(notifications)
...
export async function deleteNotification(db: DbOrTx, userId: string, id: string) {
	await db
