...
import { assertSectionAccess, canAccessSection, filterAccessibleSections } from "./checks"
...
describe("canAccessSection", () => {
  it("allows anyone into a public section, signed in or not", () =>
    withRollback(async (tx) => {
      ...
      expect(await canAccessSection(tx, scope.student, scope.publicSection)).toBe(
        true,
      )
    }))
  ...
  it("lets a user into a private section they were granted", () =>
    withRollback(async (tx) => {
      const scope = await seedSectionAccessScope(tx)
      expect(
        await canAccessSection(tx, scope.student, scope.privateGranted),
      ).toBe(true)
    }))
  ...
  it("keeps public and granted sections, drops the rest", () =>
    withRollback(async (tx) => {
      ...
      expect(allowed).toEqual(
        new Set([scope.publicSection, scope.privateGranted]),
      )
    }))
