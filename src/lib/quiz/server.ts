...
export const generateGuestQuizFn = createServerFn({ method: "GET" })
  .inputValidator(
    (input: {
      sectionId: string
  ...
  .handler(async ({ data }): Promise<Quiz | null> => {
    ...
    // Get default evaluation mode
    const { data: evalMode } = await supabase
      .from("evaluation_modes")
      .select("*")
      .limit(1)
      .single()
    ...
    const count = data.questionCount ?? 30
    const selected = selectRandomItems(questions, count)

    const quizQuestions: QuizQuestion[] = selected.map((q, i) => ({
      id: q.id,
      content: q.content,
      question_type: q.question_type,
      options: q.options
    ...
    return {
      ...
      section: section as unknown as Quiz["section"],
